...
# Configure logging
logging.basicConfig(
    filename=r"/Users/harshil/Development/Dev_tests/folder_1/DoS-Secure-DNS-Server/baseline_experimentation/logs/dns_gatekeeper.log",
    level=logging.INFO,
...
class MyDNSGatekeeper:
    ...
    def resolve(self, request, client_ip):
        query_name = str(request.question[0].name)
        query_type = dns.rdatatype.to_text(request.question[0].rdtype)
        start_time = time.time()

        # Determine server based on random key
        key = uuid.uuid4().int
        if key % 2 == 0:
            target_host, target_port = self.primary_ns_host, self.primary_ns_port
            logging.info(f"Routing query to primary server: {query_name} ({query_type}) from {client_ip}")
        else:
            target_host, target_port = self.secondary_ns_host, self.secondary_ns_port
            logging.info(f"Routing query to secondary server: {query_name} ({query_type}) from {client_ip}")

        try:
            response = self.forward_query(query_name, query_type, target_host, target_port)
            end_time = time.time()
            latency = end_time - start_time
            logging.info(f"Query resolved: {query_name} ({query_type}) in {latency:.3f} seconds")
            return response
        except Exception as e:
            logging.error(f"Failed to resolve query: {query_name} ({query_type}) - {str(e)}")
            raise
    ...
    def validate(self, sender_ip):
        if sender_ip in self.history:
            ...
            if self.history[sender_ip] > 100:
                logging.warning(f"IP {sender_ip} exceeded rate limit")
                return False
            return True
        else:
            self.history[sender_ip] = 1
    ...
    def reset_history(self):
        try:
            while True:
                time.sleep(5)
                self.history = {}
                logging.info("Rate limiting history reset")
    ...
    def run(self):
        try:
            while True:
                ...
                if not self.validate(client_ip):
                ...
                try:
                    reply = self.resolve(request, client_ip)
                except Exception:
                    continue
    ...
    def perform_zone_transfers(self):
        try:
            while True:
                time.sleep(100)
                logging.info("Initiating zone transfer")
    ...
    def zone_transfer(self, host, port, target_host, target_port):
        ...
        msg = f"ZONE_TRANSFER example.com {target_host} {target_port}"
        client_socket.sendto(msg.encode('utf-8'), (host, port))

        try:
            response, server_address = client_socket.recvfrom(1024)
            decoded_response = response.decode('utf-8')
            logging.info(f"Zone transfer response from {server_address}: {decoded_response}")
        except socket.timeout:
            logging.error("Zone transfer timed out")
        finally:
            client_socket.close()
